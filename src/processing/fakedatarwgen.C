...
int fakedatarwgen(std::string const &ifile, std::string const &ofile) {
  ...
  for (std::string const &species : {"numu", "numub", 
    //"nue", "nueb"
    }) {
      ...
      for (t2knova::selection sel_id : ReWeightSelectionList) {
        ...
        if (neut_nd280_EnuPLepThetaLep) { // ND280
          ...
          for (int i = 0;
            ...
            double int_NEUT_TO_GENIE = IntegralTH2(PLepThetaLep_NEUT_to_GENIE);
        ...
        if (neut_nd280_Enu) { // ND280
          ...
          std::unique_ptr<TH1> genie_nd280 =
              GetTH1(fin, "GENIE/ND280/" + targetnuc + "/" + species + "/Enu_" +
                              selection);
          genie_nd280->SetDirectory(nullptr);
        ...
        if (neut_nd280_Q2) { // ND280
          ...
          std::unique_ptr<TH1> genie_nd280 =
              GetTH1(fin, "GENIE/ND280/" + targetnuc + "/" + species + "/Q2_" +
                              selection);
          genie_nd280->SetDirectory(nullptr);
        ...
        if (neut_novand_plep) { // NOvAND
          ...
          if (DoExtra) {
            ...
            dir->WriteTObject(nova_to_t2k_yx.get(), nova_to_t2k_yx->GetName());
            dir->WriteTObject(nova_to_t2k_zx.get(), nova_to_t2k_zx->GetName());
            dir->WriteTObject(nova_to_t2k_yz.get(), nova_to_t2k_yz->GetName());
          }
          std::unique_ptr<TH3> rat(
              dynamic_cast<TH3 *>(neut_novand_plep->Clone(selection.c_str())));
          rat->Divide(genie_novand.get());
          ScrubLowStatsBins(neut_novand_plep, genie_novand, rat, MaxFracError);
          rat->SetDirectory(nullptr);
          dir->WriteTObject(rat.get(), rat->GetName());
        } else {
          std::cout << "[WARN]: Expected to find "
        ...
        if (neut_novand_q2) { // NOvAND
          ...
          if (DoExtra) {
            std::unique_ptr<TH2> nova_to_t2k_yx =
        ...
        if (neut_novand_ptlep) { // NOvAND
          ...
          std::unique_ptr<TH3> rat(
              dynamic_cast<TH3 *>(neut_novand_ptlep->Clone(selection.c_str())));
          rat->Divide(genie_novand.get());
          ScrubLowStatsBins(neut_novand_ptlep, genie_novand, rat, MaxFracError);
          rat->SetDirectory(nullptr);
          dir->WriteTObject(rat.get(), rat->GetName());
        } else {
          std::cout << "[WARN]: Expected to find "
